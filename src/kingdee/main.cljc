(ns kingdee.main
  "Kotodama WASM entrypoint for the Kingdee clean-room actor (L4) — Clojure port.
  ...
  facts (`kingdee.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kingdee")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"kingdee.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kingdee-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kingdee-compat/ready)
